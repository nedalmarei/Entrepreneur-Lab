== Zita Elementor==
Contributors: wpzita
Tags: two-columns, right-sidebar, block-patterns, custom-menu, custom-background, editor-style, translation-ready, full-width-template, flexible-header, sticky-post, theme-options, one-column, e-commerce, blog, photography, threaded-comments, custom-colors
Requires at least: 5.0
Tested up to: 6.1
Stable tag: 1.5.8
License: GPLv3 or later
License URL: https://www.gnu.org/licenses/gpl-3.0.en.html

Zita elementor theme is Easy to use, Fully Customizable, it has unique option which helps to create great website.

== Description ==
Zita is super fast and highly customizable Elementor theme. Suitable to creating any type of website such a shopping,  corporate, store, agency, blog, portfolio, restaurant, wedding, construction, hospital, school, interior, car workshop, life coach, wine shop, barber shop, furniture shop, yoga, resume, charity, digital marketing  etc. Zita include 8 unique header layout and 8 footer widget layout which make it extremely powerful. Zita works with any of your favourite page builder like Elementor, Visual Composer, Beaver Builder, SiteOrigin, Divi, Brizy etc. Some more amazing feature as Woocommerce ready, RTL Ready, Full Responsive, Edit settings in tablet & mobile, Top header, Bottom header, Top footer, Bottom Footer, Transparent Header. Looking for perfect base theme? Looking for perfect multipurpose theme? Look no further! just check Zita demos:https://wpzita.com/. Zita is built with best SEO practice and best friend of Woocommerce and Elementor.

For more information about Zita please go to : https://wpzita.com

== Theme License & Copyright ==
Zita WordPress Theme, Copyright 2019 wpzita.com
Zita is distributed under the terms of the GNU General Public License v3
License : GPLv3 or later
License URL: https://www.gnu.org/licenses/gpl-3.0.en.html

== Resources ==

License for images:

1. Image Name: screenshot.png
Resource link: https://burst.shopify.com/photos/portrait-of-a-young-man-smiling?c=man
Licensed under the CCO license.
License link : https://burst.shopify.com/licenses/creative-commons

== Catamaran Font ==
License: SIL OFL 1.1
License URL: http://scripts.sil.org/OFL

== FontAwesome License ==
License: SIL OFL 1.1
License URl for Font : http://scripts.sil.org/cms/scripts/page.php?site_id=nrsi&id=OFL

== Font Awesome CSS license ==
License: MIT License
License URL for CSS : http://opensource.org/licenses/mit-license.html

== normalize.css == 
License: MIT
Source: https://necolas.github.io/normalize.css/

== imagesloaded.pkgd.js ==
Licensed under the MIT license
License URL for CSS : http://opensource.org/licenses/mit-license.html

== masonry.pkgd.js ==
license : GNU GPL license v3
License URL : https://masonry.desandro.com

== breadcrumb.php ==
breadcrumbs.php, Copyright Justin Tadlock License: GNU GPL License, version 0.2.1 Source: https://github.com/justintadlock/breadcrumb-trail/

==  Other Licenses ==
Other js files are self coded.

== Frequently Asked Questions ==

= Does your theme support builder =
Yes our theme supports builder

= Can i use your theme with latest version of WordPress=
Yes, this theme is tested with latest version of WordPress, you can use it for your site.

== Changelog ==

= 1.5.8=
* Block Pattern Added.

= 1.5.7=
* Theme tested With WordPress 6.1.

= 1.5.6=
* Style issue fixd.
* Search color and margin issue fixd.

= 1.5.5=
* bug fixed.
* style fix.

= 1.5.4=
* Option panel new button added.
* style fix.

= 1.5.3=
* HTML markup issue resolved.

= 1.5.2=
* Page Header functionality Added.
* Learnpress plugin compatability added.
* Menu in the below header not working issue resolved.
* Transparent header in blog page issue resolved.
* Menu in the top header not working issue resolved.
* Font Weight style removed from strong tag.
* Loading issue in firefox resolved.
* Upsell products issue resolved.
* Review secion in single product style changed.

= 1.5.1=
* Footer Text Issue Fixed.

= 1.5.0 =
* Wordpress 5.7 localize issue fixed.
* Search in mobile view issue fixed.

= 1.4.9 =
* Comment Pagination numbers added.

= 1.4.8 =
* Comment Pagination Issue Resolved.

= 1.4.7 =
* Theme color option improved.
* Page meta box issue fixed.

= 1.4.6 =
* Compitable with latest wordpress.

= 1.4.5 =
* Style issue fixed.

= 1.4.4 =
* Header responsive style improved.

= 1.4.3 =
* Mobile Pan scrolling issue fixed.

= 1.4.2 =
* Add to cart button in grouped product issue fixed.
* Style issue fixed.

= 1.4.1 =
* Footer copyright issue fixed.
* Style issue fixed.

= 1.4.0 =
* style improved.

= 1.3.9 =
* Keyboard navigation improved.

= 1.3.8 =
* Header bottom spacing issue fixed.
* WP 5.4 compatibility checked.

= 1.3.7 =
* Header issue fixed.

= 1.3.6 =
* sticky header jerk issue fixed.

= 1.3.5 =
* iframe height issue fixed.

= 1.3.4 =
* Social icon style issue fixed.
* Sanitize issue fixed.

= 1.3.3 =
* Input field focus issue fixed.

= 1.3.2 =
* Site title changed to H1 for blog page.

= 1.3.1 =
* Mobile Tab Navigation Issue resolved.

= 1.3.0 =
* Header Transparent Issue resolved.
* Mobile / Pan Color Issue Fixed.

= 1.2.9 =
* Typography Updated.

= 1.2.8 =
* Archive Pagination Issue Fixed.

= 1.2.7 =
* Post Editor Excerpt Option Added.
* Single Product Sidebar Issue Fixed.

= 1.2.6 =
* Post Pagination issue fixed.

= 1.2.5 =
* google plus social icon removed.

= 1.2.4 =
* sidebar bug fixed.

= 1.2.3 =
* Shop page sidebar alignment bug fixed.
* Compare product and Wishlist styling improved.
* Post comment styling improved.

= 1.2.2 =
* Comment Nav Link Issue Resolved.

= 1.2.1 =
* Admin Notice Style Changed.

= 1.2.0 =
* Add Skip Content Link.

= 1.1.10 =
* optimize issue fix.

= 1.1.9 =
* Mobile Menu open issue fix.
* Option panel style issue fix.

= 1.1.8 =
* Theme option change.
* Style issue fix.

= 1.1.7 =
* Style fix.

= 1.1.6 =
* Menu issue fix.
* Page builder template issue fix.

= 1.1.5 =
* Readme file updated

= 1.1.4 =
* WooCommerce Single page updated

= 1.1.3 =
* Smooth Scrolling issue fixed.
* Single Post content issue fixed.

= 1.1.2 =
* Style issue fixed.

= 1.1.1 =
* Style issue fixed.
* 404 page style improved.

= 1.1.0 =
* Style issue fixed.

= 1.0.9 =
* Bug fixed.

= 1.0.8 =
* Bug fixed.

= 1.0.7 =
* Option panel updated.

= 1.0.6 =
* Pagination issue fixed.

= 1.0.5 =
* Option panel improved.

= 1.0.4 =
* Issue Fixed.
* Header layout updated.

= 1.0.3 =
* subject tags updated.

= 1.0.2 =
* Readme.txt file updated.

= 1.0.1 =
* style issue fixed.
* error fixed.

= 1.0 =
* Released: Sep 18, 2018

Initial release

== Upgrade Notice ==

= 1.5.8=
* Block Pattern Added.